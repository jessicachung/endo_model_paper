...
pheno <- sample_info %>% 
  filter(cycle_stage %in% 1:7,
         sample_id %in% colnames(combat_exprs),
         study == "study_1") %>%
...
```{r}
exprs <- combat_exprs[,pheno$sample_id]
...
```{r}
ggplot(model_df %>% head(5000), aes(x=spline_edf, y=r2)) +
  geom_point(alpha=0.2, size=1)
```

```{r}
ggplot(model_df %>% head(5000), aes(x=dev_exp)) +
...
```{r}
# Get best estimate
fit2_best_estimate <- sapply(colnames(combat_exprs), function(s) {
  sweep(x=expected_exprs, MARGIN=1, STATS=combat_exprs[,s]) %>% 
    `^`(2) %>% colSums %>% which.min %>% names %>% str_replace("time_", "") %>%
    as.numeric
})
...
array_samples <- colnames(combat_exprs)[colnames(combat_exprs) %in% combat_phenotype$sample_id]
m <- match(array_samples, combat_phenotype$sample_id)
...
saveRDS(combat_phenotype,
        file=sprintf("data/tidy_data/array_combat_phenotype_%s.rds", unique_id))
saveRDS(fit2_best_estimate,
        file=sprintf("cache/array_fit2_best_estimate_%s.rds", unique_id))
saveRDS(f,
        file=sprintf("cache/array_splinefun_%s.rds", unique_id))
```
